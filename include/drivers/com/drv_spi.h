/*
 * drv_spi.h — Generic SPI driver types, ops table, handle and public API
 *
...
typedef struct { SPI_HandleTypeDef hspi; } drv_hw_spi_ctx_t;
#else
typedef struct { uint32_t _reserved; } drv_hw_spi_ctx_t;
#endif
...
typedef struct drv_spi_hal_ops {
    ...
    /* IT variants — completion via IRQ_ID_SPI_TX/RX/TXRX_DONE */
    int32_t (*transmit_it)(drv_spi_handle_t *h, const uint8_t *data,
                            uint16_t len);
    int32_t (*receive_it) (drv_spi_handle_t *h, uint8_t *data, uint16_t len);
    int32_t (*transfer_it)(drv_spi_handle_t *h, const uint8_t *tx,
