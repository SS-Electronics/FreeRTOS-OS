# Debug Framework — FreeRTOS-OS {#debug-framework--freertos-os}
...
| `arch/debug/target/stm32_f411xx_debug.cfg` | OpenOCD target script — flash bank, reset strategy, event hooks |
...
### Software — install with the provided scripts {#software--install-with-the-provided-scripts}
...
2. Downloads `STM32F411.svd` from the `posborne/cmsis-svd` repository into `arch/debug/target/`
...
### 1. Build, Flash and Debug {#1-build-flash-and-debug}
...
```jsonc
{
    ...
    "interface": "swd",
    "runToEntryPoint": "main",
    "svdFile": "${workspaceFolder}/arch/debug/target/STM32F411.svd",
    "rtos": "FreeRTOS",
    "preLaunchTask": "detect-target"   // runs gen_active_debug.py then build
}
```

> `build/active_debug.cfg` is auto-generated by the `detect-target` task before every launch. It reads `CONFIG_TARGET_MCU` from `config/autoconf.h` and emits an OpenOCD `source {}` line pointing to the correct `arch/debug/target/*.cfg` file. This means switching targets requires no manual edits to `launch.json`.
...
### `openocd-server` task (background) {#openocd-server-task-background}
...
**File:** [`arch/debug/target/stm32_f411xx_debug.cfg`](arch/debug/target/stm32_f411xx_debug.cfg)
...
### SWD Clock Speed {#swd-clock-speed}
...
### RTOS Threads panel {#rtos-threads-panel}
...
### Source-level FreeRTOS stepping {#source-level-freertos-stepping}
...
### Stack high-water mark visibility {#stack-high-water-mark-visibility}
...
**File:** `arch/debug/target/STM32F411.svd` (downloaded by `install_debug_tools.sh`)
...
### Enabling in launch.json {#enabling-in-launchjson}
...
### Enabling in firmware {#enabling-in-firmware}
...
### Live variable watch {#live-variable-watch}
...
### Start OpenOCD server manually {#start-openocd-server-manually}
...
### Connect GDB manually {#connect-gdb-manually}
...
### ST-Link not found / USB error {#st-link-not-found--usb-error}
...
### Target not halting / `connect_assert_srst` fails {#target-not-halting--connect_assert_srst-fails}
...
### GDB reports wrong variable values after rebuild {#gdb-reports-wrong-variable-values-after-rebuild}
...
### `libusb_bulk_transfer` errors / CRC errors {#libusb_bulk_transfer-errors--crc-errors}
...
### Peripherals panel empty (no SVD) {#peripherals-panel-empty-no-svd}

```
arch/debug/target/STM32F411.svd: No such file or directory
```
- Run `bash scripts/install_debug_tools.sh`
- Or manually download and place the file

### FreeRTOS threads not showing {#freertos-threads-not-showing}
...
### SWO output is garbled {#swo-output-is-garbled}
...
### Different STM32F4 variant {#different-stm32f4-variant}

1. Copy `arch/debug/target/stm32_f411xx_debug.cfg` to a new file
...
### Different debug probe (J-Link) {#different-debug-probe-j-link}
...
### Different vendor (non-STM32) {#different-vendor-non-stm32}
