...
```jsonc
{
    ...
    "request": "launch",
    "servertype": "openocd",
    "configFiles": ["arch/debug_cfg/stm32_f411xx_debug.cfg"],
...
board-gen ──────────────────────────────────────────────────────┐
  ...
  → boards/<board>/board_config.c                               ▼
  → include/board/board_device_ids.h                        build (default: Ctrl+Shift+B)
                                                               make all
config-outputs ─────────────────────────────────────────────────┤  (auto-runs board-gen if XML newer)
  make config-outputs                                            │  → build/kernel.elf
  → include/config/autoconf.h                                   │  → build/kernel.elf.hex
...
- `cpuFrequency` must exactly match the actual HCLK. Check `system_stm32f4xx.c` for the configured PLL output (100 MHz for STM32F411 with 25 MHz HSE and the default `.config`)
