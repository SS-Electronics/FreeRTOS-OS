...
**The vendor is selected at compile time** via `CONFIG_DEVICE_VARIANT` in [`app/board/mcu_config.h`](../app/board/mcu_config.h) (generated by `scripts/gen_board_config.py`):
...
> Interrupt handler declarations are **not** in `device.h`. They live in [`include/drivers/hal/stm32/hal_it_stm32.h`](include/drivers/hal/stm32/hal_it_stm32.h), included directly by `hal_it_stm32.c`.
...
### RCC Driver and System Initialisation
...
#### `hal_rcc_stm32.c` — STM32 HAL backend
...
#### `drv_rcc.c` / `drv_rcc.h` — generic RCC dispatch
...
1. **Add a `MCU_VAR_*` constant** in [`app/board/mcu_config.h`](../app/board/mcu_config.h) (generated — add it to `scripts/gen_board_config.py`).
