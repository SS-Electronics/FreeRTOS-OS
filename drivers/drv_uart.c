 ...
 *
 * *****************************************************/
void drv_serial_update_handle(type_drv_hw_handle * handle)
{
	if(handle != NULL)
	{
		uart_handle_ref = handle;
	}
}
...
status_type	drv_serial_init(uint8_t dev_id)
{
	...
	if(uart_handle_ref->handle_hw_uart[dev_id] != NULL)
	{
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if(huart == uart_handle_ref->handle_hw_uart[dev_id])
	{
