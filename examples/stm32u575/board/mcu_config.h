/*
 * mcu_config.h — MCU-level peripheral counts for STM32U575ZITxQ devboard.
 *
 * Board: NUCLEO-U575ZI-Q   MCU: STM32U575ZITxQ (Cortex-M33 + TrustZone,
 *                                                 160 MHz, 2 MB flash,
 *                                                 786 KB SRAM)
 *
 * This file is the example-build equivalent of the app/board/mcu_config.h
 * that would normally be generated by gen_board_config.py.  It is tracked
 * in the repository so the example build has no dependency on the sibling
 * app/ repository.
 *
 * If you change the board peripherals in stm32u575_devboard.xml, regenerate:
 *   make dev-stm32u575
 */

#ifndef BOARD_MCU_CONFIG_H_
#define BOARD_MCU_CONFIG_H_

#include "autoconf.h"

/* ── MCU Vendor / Variant Selection ──────────────────────────────────────── */
#define MCU_VAR_MICROCHIP   1
#define MCU_VAR_STM         2
#define MCU_VAR_INFINEON    3

#define CONFIG_DEVICE_VARIANT    MCU_VAR_STM   /* STM32U575ZITxQ */

/* ── Hardware-fixed peripheral instance counts ───────────────────────────── */
#define MCU_MAX_UART_PERIPHERAL    6   /* USART1..3 + UART4..5 + LPUART1   */
#define MCU_MAX_IIC_PERIPHERAL     4   /* I2C1..I2C4                       */
#define MCU_MAX_SPI_PERIPHERAL     3   /* SPI1..SPI3                       */
#define MCU_MAX_TIM_PERIPHERAL    17   /* TIM1..TIM8, TIM15..17, LPTIM1..4 */

/* ── Board device IDs — generated from XML by gen_board_config.py ─────────── */
#include <board/board_device_ids.h>

/* ── OS-managed peripheral counts — sourced from board config ────────────── */
#ifdef BOARD_UART_COUNT
  #define NO_OF_UART    BOARD_UART_COUNT
#else
  #define NO_OF_UART    1
#endif

#ifdef BOARD_IIC_COUNT
  #define NO_OF_IIC     BOARD_IIC_COUNT
#else
  #define NO_OF_IIC     0
#endif

#ifdef BOARD_SPI_COUNT
  #define NO_OF_SPI     BOARD_SPI_COUNT
#else
  #define NO_OF_SPI     0
#endif

#ifdef BOARD_GPIO_COUNT
  #define NO_OF_GPIO    BOARD_GPIO_COUNT
#else
  #define NO_OF_GPIO    0
#endif

/* ── Peripheral enable flags ─────────────────────────────────────────────── */
#ifdef CONFIG_HAL_IWDG_MODULE_ENABLED
  #define CONFIG_MCU_WDG_EN               (1)
#else
  #define CONFIG_MCU_WDG_EN               (0)
#endif

#define CONFIG_MCU_FLASH_DRV_EN           (1)

#define CONFIG_MCU_NO_OF_UART_PERIPHERAL  NO_OF_UART
#define CONFIG_MCU_NO_OF_IIC_PERIPHERAL   NO_OF_IIC
#define CONFIG_MCU_NO_OF_SPI_PERIPHERAL   NO_OF_SPI
#define CONFIG_MCU_NO_OF_GPIO_PERIPHERAL  NO_OF_GPIO

#define CONFIG_MCU_NO_OF_RS485_PERIPHERAL (0)
#define CONFIG_MCU_NO_OF_CAN_PERIPHERAL   (0)
#define CONFIG_MCU_NO_OF_USB_PERIPHERAL   (0)
#define CONFIG_MCU_NO_OF_ETH_PERIPHERAL   (0)

#ifdef CONFIG_HAL_TIM_MODULE_ENABLED
  #define CONFIG_MCU_NO_OF_TIMER_PERIPHERAL (2)
#else
  #define CONFIG_MCU_NO_OF_TIMER_PERIPHERAL (0)
#endif

#define NO_OF_TIMER   CONFIG_MCU_NO_OF_TIMER_PERIPHERAL

/* ── Individual UART instance enable ─────────────────────────────────────── */
#define UART_1_EN    (NO_OF_UART >= 1 ? 1 : 0)
#define UART_2_EN    (NO_OF_UART >= 2 ? 1 : 0)
#define UART_3_EN    (0)
#define UART_4_EN    (0)
#define UART_5_EN    (0)
#define UART_6_EN    (0)   /* LPUART1 is UART_6 slot; controlled below */
#define UART_7_EN    (0)
#define UART_8_EN    (0)

/* ── Peripheral identifier enumerations ─────────────────────────────────── */
typedef enum {
    UART_1 = 0,    /* USART1  */
    UART_2,        /* USART2  */
    UART_3,        /* USART3  */
    UART_4,        /* UART4   */
    UART_5,        /* UART5   */
    UART_6,        /* LPUART1 (Nucleo VCP) */
    UART_7,
    UART_8
} UART_PORTS;

typedef enum {
    IIC_1 = 0,
    IIC_2,
    IIC_3,
    IIC_4
} IIC_PORTS;

typedef enum {
    TIMER_1,
    TIMER_2,
    TIMER_3,
    TIMER_4,
    INVALID_TIMER_ID
} TIMER_PORTS;

#endif /* BOARD_MCU_CONFIG_H_ */
