...
/* SCB_EnableICache / SCB_EnableDCache from core_cm7.h via stm32h7xx.h → stm32_h723xx.h.
 * -I arch/devices/STM/STM32H7xx resolves <stm32h7xx.h>; -DSTM32H723xx selects sub-header. */
#include <stm32h7xx.h>
...
/* Cortex-M7 exception handlers (weak aliases to Default_Handler) */
...
__asm volatile (
    ...
    /* Enable Cortex-M7 instruction and data caches before any memory access.
     * Must happen before SystemInit so PLL/flash-wait-state setup benefits
     * from cache; SCB_EnableICache/SCB_EnableDCache are no-ops if already on. */
    SCB_EnableICache();
    SCB_EnableDCache();

    /* System init (clocks, VOS, PLL, flash latency, etc.) */
    SystemInit();
    ...
    /* Copy generic .data section from flash to RAM */
